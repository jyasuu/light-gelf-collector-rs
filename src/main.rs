    ...
    loop {
        ...
        match socket.recv_from(&mut buf).await {
            Ok((len, addr)) => {
                ...
                debug!("Message preview (first 200 chars): {}", 
...
fn parse_gelf_message(message_str: &str) -> Result<GelfMessage, serde_json::Error> {
    ...
    match serde_json::from_str::<GelfMessage>(message_str) {
        Ok(gelf_msg) => {
            ...
            debug!("Parsed message fields - version: {:?}, host: {:?}, short_message length: {}", 
                   gelf_msg.version, 
                   gelf_msg.host, 
        ...
        Err(e) => {
            ...
            debug!("Failed to parse as GELF, message preview: {}", 
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let args = Args::parse();
    debug!("Parsed command line arguments: UDP port: {}, HTTP port: {}, bind address: {}, max messages: {}", 
